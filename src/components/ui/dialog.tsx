...
const DialogContent = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, DialogContentProps<T>>) => {
  const [{ class: className, children, lang }, rest] = splitProps(props as DialogContentProps, [
    'class',
    'children',
    'lang',
  ])

  // Determine RTL direction based on language
  const isRTL = createMemo(() => lang === 'ar')
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        class={cn(
          'fixed left-1/2 top-1/2 z-50 grid max-h-screen w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 overflow-y-auto border bg-background p-6 shadow-lg duration-200',
          ...
          'sm:rounded-lg',
          isRTL() ? 'rtl' : 'ltr', // Add RTL class based on language
          className
        )}
      ...
      >
        ...
        <DialogPrimitive.CloseButton
          class={cn(
            'absolute opacity-70 ring-offset-background transition-opacity hover:opacity-100',
...
const DialogHeader: Component<ComponentProps<'div'> & { lang?: string }> = (props) => {
  const [{ class: className, lang }, rest] = splitProps(props, ['class', 'lang'])
  const isRTL = createMemo(() => lang === 'ar')
  ...
  return (
    <div
      class={cn(
        ...
        isRTL() ? 'sm:text-right text-right' : 'sm:text-left text-center',
        className
      )}
      dir={isRTL() ? 'rtl' : 'ltr'}
      {...rest}
...
const DialogFooter: Component<ComponentProps<'div'> & { lang?: string }> = (props) => {
  const [{ class: className, lang }, rest] = splitProps(props, ['class', 'lang'])
  const isRTL = createMemo(() => lang === 'ar')
...
const DialogTitle = <T extends ValidComponent = 'h2'>(props: PolymorphicProps<T, DialogTitleProps<T>>) => {
  const [{ class: className, lang }, rest] = splitProps(props as DialogTitleProps, ['class', 'lang'])
  const isRTL = createMemo(() => lang === 'ar')

  return (
    <DialogPrimitive.Title
      class={cn('text-lg font-semibold leading-none tracking-tight', isRTL() ? 'text-right' : 'text-left', className)}
      dir={isRTL() ? 'rtl' : 'ltr'}
      {...rest}
    />
  )
...
const DialogDescription = <T extends ValidComponent = 'p'>(props: PolymorphicProps<T, DialogDescriptionProps<T>>) => {
  const [{ class: className, lang }, rest] = splitProps(props as DialogDescriptionProps, ['class', 'lang'])
  const isRTL = createMemo(() => lang === 'ar')

  return (
    <DialogPrimitive.Description
      class={cn('text-sm text-muted-foreground', isRTL() ? 'text-right' : 'text-left', className)}
      dir={isRTL() ? 'rtl' : 'ltr'}
      {...rest}
    />
  )
