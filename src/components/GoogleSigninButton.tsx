import { Component, Show } from 'solid-js'
import { createAsync } from '@solidjs/router'
import { useAuth } from '@solid-mediakit/auth/client'
import { query } from '@solidjs/router'
import { getSession } from '@solid-mediakit/auth'
import { getRequestEvent } from 'solid-js/web'
import { authOptions } from '~/lib/auth/authOptions'

// Server query to get user session
const getUserSession = query(async () => {
  'use server'
  const event = getRequestEvent()!
  const session = await getSession(event, authOptions)
  return session
}, 'user-session')

const AuthShowcase: Component = () => {
  const auth = useAuth()
  const session = createAsync(() => getUserSession(), { deferStream: true })

  const handleSignIn = async () => {
    try {
      await auth.signIn('google', {
        redirect: false,
        redirectTo: window.location.pathname,
      })
    } catch (error) {
      console.error('Google sign-in failed:', error)
    }
  }

  const handleSignOut = async () => {
    try {
      await auth.signOut({ redirect: false })
    } catch (error) {
      console.error('Sign out failed:', error)
    }
  }

  return (
    <div class='flex flex-col items-center gap-4 p-6 rounded-lg bg-white shadow-md'>
      <Show
        when={session()}
        fallback={
          <button
            onClick={handleSignIn}
            class='flex items-center justify-center gap-3 
                   px-6 py-2 rounded-md
                   bg-white hover:bg-gray-50 
                   text-gray-800 font-medium
                   border border-gray-200 
                   w-full max-w-sm'
          >
            <svg class='w-5 h-5' viewBox='0 0 24 24'>
              <path
                fill='#4285F4'
                d='M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z'
              />
              <path
                fill='#34A853'
                d='M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z'
              />
              <path
                fill='#FBBC05'
                d='M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z'
              />
              <path
                fill='#EA4335'
                d='M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z'
              />
            </svg>
            Sign in with Google
          </button>
        }
      >
        <div class='flex flex-col items-center gap-4'>
          <div class='flex items-center gap-4'>
            <Show when={session()?.user?.image}>
              <img src={session()?.user?.image} alt='Profile' class='w-12 h-12 rounded-full' />
            </Show>
            <div class='flex flex-col'>
              <span class='font-medium text-lg'>{session()?.user?.name}</span>
              <span class='text-gray-500 text-sm'>{session()?.user?.email}</span>
            </div>
          </div>

          <button
            onClick={handleSignOut}
            class='px-4 py-2 rounded-md
                   bg-red-500 hover:bg-red-600 
                   text-white font-medium
                   transition-colors duration-200'
          >
            Sign Out
          </button>
        </div>
      </Show>
    </div>
  )
}

export default AuthShowcase
