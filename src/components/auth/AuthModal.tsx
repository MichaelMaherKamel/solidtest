...
const AuthModal: Component<AuthModalProps> = (props) => {
  ...
  const handleSignIn = async (provider: 'google') => {
    ...
    try {
      setLoading(provider)
      setError('')
      await auth.signIn(provider) // Simplified sign-in
      // The redirect is handled automatically by useAuth
    } catch (err) {
      ...
      >
        <div class='p-6 space-y-6'>
          ...
          <div class='space-y-4'>
            <Button
              variant='outline'
              class='w-full h-12 relative bg-white hover:bg-gray-50 border-gray-300 transition-all duration-300 group overflow-hidden'
