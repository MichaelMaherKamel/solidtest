...
const AuthModal: Component<AuthModalProps> = (props) => {
  ...
  const handleSignIn = async (provider: 'google') => {
    ...
    try {
      ...
      const popup = window.open('', 'oauthPopup', 'width=600,height=600')
      ...
      // Poll for the callback URL (adjust based on your auth flow)
      const checkInterval = setInterval(() => {
        if (popup?.closed) {
          clearInterval(checkInterval)
          props.onOpenChange(false)
          window.location.reload() // Refresh after successful auth
        }
      }, 500)
    } catch (err) {
